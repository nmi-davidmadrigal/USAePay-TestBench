...
public sealed partial class RedactionService
{
    ...
    [GeneratedRegex("\"(?<key>cardNumber|pan|cvv|cvc|securityCode|trackData|track1|track2|authorization|apiKey|apiSecret|accessToken|paymentKey|token)\"\\s*:\\s*\"(?<value>[^\"]*)\"", RegexOptions.IgnoreCase)]
    private static partial Regex JsonSensitiveKeyRegex();

    [GeneratedRegex("<(?<key>cardNumber|pan|cvv|cvc|securityCode|trackData|track1|track2|authorization|apiKey|apiSecret|accessToken|paymentKey|token)>(?<value>[^<]*)</\\k<key>>", RegexOptions.IgnoreCase)]
